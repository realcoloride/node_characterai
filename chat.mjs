...
import { Reply, Message, MessageHistory, OutgoingMessage } from "./message.mjs";
...
export class Chat {
  ...
  async uploadImage(content) {
    ...
    if (request.status() === 200 && response.status == "OK") {
      ...
      throw Error(
        `Failed uploading image: ${response.error || response.detail}`
      );
  }
  ...
  async changeToConversationId(conversationExternalId, force = false) {
    ...
    if (passing) this.externalId = conversationExternalId;
      ...
      Error(
        "Could not switch to conversation, it either doesn't exist or is invalid."
      );
  }
  ...
  async deleteMessage(messageId) {
    ...
    if (this.client.isGuest())
      throw Error("Guest accounts cannot delete messsages.");
  ...
  async deleteMessagesBulk(
    ...
    if (!this.client.isAuthenticated())
      throw Error("You must be authenticated to do this.");
    if (this.client.isGuest())
      throw Error("Guest accounts cannot bulk delete messsages.");
    ...
    for (let i = 0; i < pagesToFetch; i++) {
      ...
      if (printSteps == true)
        console.log(
          `[node_characterai] Bulk delete - Current page: ${999999 - i}`
        );
